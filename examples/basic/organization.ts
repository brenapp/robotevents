/**
 * Gets the record for a given organization.
 **/
...
const LETTERS = ["A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "M", "N", "P", "R", "S", "T", "U", "V", "W", "X", "Y", "Z"];


(async function () {

    const VRC = robotevents.programs.get("VRC");

    const number = "3796";
    const numbers = LETTERS.map(letter => `${number}${letter}`);
    ...
    for (const team of teams) {
        ...
        const matches = await team.matches({
            season: [robotevents.seasons.get("VRC", "2022-2023")!],
        });

        const outcomes = matches.group(match => match.teamOutcome(team.number));
        wins += outcomes.win?.length ?? 0;
        ties += outcomes.tie?.length ?? 0;
        losses += outcomes.loss?.length ?? 0;
    };
