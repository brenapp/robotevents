...
const SpinUpVRC = robotevents.seasons.current("VRC")!;

(async function () {

    const event = await robotevents.events.get("RE-VRC-22-7740");
    if (!event) return;

    const matches = await event.matches(1);
    const groupedMatches = matches.group(match => match.round);
